...
#         return 1 # for testing
    ...
    def __getitem__(self, ind):
        ...
        if self.input_images[str(ratio)[0:3]][ind] is None:
            ...
            save_gt = np.expand_dims(np.float32(im / 65535.0), axis = 0)
            self.input_images[str(ratio)[0:3]][ind] = save_gt

            gt_raw = rawpy.imread(gt_path)
            im = gt_raw.postprocess(use_camera_wb=True, half_size=False, no_auto_bright=True, output_bps=16)
            save_gt = np.expand_dims(np.float32(im / 65535.0), axis = 0)
            self.gt_images[ind] = save_gt
        ...
        gt_patch = self.gt_images[ind][:, yy:yy+ps, xx:xx+ps, :]
